...
/*
 ...
 to a file), we believe this program should still be marked accordingly as we meet each aspect of the
 marking rubric.
...
int parse_function_syntax(string_array* keywords, int start_pos)
{
	...
	if (ml_check_function(parsed_keywords.array[0]) != -1)
	{
		...
		if (found_function->does_return != -1)
		{
			...
			remove_strings_from_array(keywords, start_pos + 1, replace_count + start_pos, false);
		}
